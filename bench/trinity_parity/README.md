...
- Raptor `trinity` workflow command, report path, component clustering mode, component JSON path/count, component graph JSON path/count, graph node/edge counts, edge read/pair/k-mer support, read k-mer node/edge counts, capped read k-mer edge sample count, component assigned read/pair counts, and recovery metrics when requested
