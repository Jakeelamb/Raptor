...
#[test]
fn gfa_link_parsing_is_stable_under_link_record_order() {
    let mut file_a = NamedTempFile::new().unwrap();
    writeln!(file_a, "H\tVN:Z:1.0").unwrap();
    writeln!(file_a, "S\tseg_a\tACGT").unwrap();
    writeln!(file_a, "S\tseg_b\tACGT").unwrap();
    writeln!(file_a, "S\tseg_c\tACGT").unwrap();
    ...
    writeln!(file_b, "S\tseg_a\tACGT").unwrap();
    writeln!(file_b, "S\tseg_b\tACGT").unwrap();
    writeln!(file_b, "S\tseg_c\tACGT").unwrap();
