#!/usr/bin/env python3
"""Promote a read-mapping candidate through local eval and quick_test."""
...
DEFAULT_OUTPUT_ROOT = (
    REPO_ROOT / "artifacts" / "autoresearch_raptor" / "read_mapping" / "promotions"
)
...
QUICK_TEST_REFERENCE = REPO_ROOT / "bench" / "genome_assembly" / "data" / "quick_test" / "reference.fa"
...
class PromotionGate:
    ...
    min_heldout_scaffold_specificity_delta: float = 0.0
    max_heldout_scaffold_specificity_drop: float = 0.0
...
def load_config(args: argparse.Namespace) -> MapperConfig:
    if args.train_result is not None and args.train_result.exists():
        ...
        if winner:
            return MapperConfig(
                k=winner["k"],
                w=winner["w"],
                min_primary_matches=winner["min_primary_matches"],
                min_scaffold_matches=winner["min_scaffold_matches"],
            )

    return MapperConfig(
        k=args.k,
        w=args.w,
        min_primary_matches=args.min_primary_matches,
        min_scaffold_matches=args.min_scaffold_matches,
    )
...
def evaluate_read_mapping_panel(
    binary: Path,
    task_root: Path,
    config: MapperConfig,
...
def safe_get_metric(report: dict[str, Any] | None, key: str) -> float | int | None:
    if report is None:
        return None
    ...
    if key == "scaffold_specificity_rate":
        return report.get("primary_specificity_rate")
    return None
...
def run_quick_test(
    ...
    cmd = [
        ...
        "--read-mapping-w",
        str(config.w),
        "--read-mapping-min-primary-matches",
        str(config.min_primary_matches),
        "--read-mapping-min-scaffold-matches",
        str(config.min_scaffold_matches),
    ]
...
def compare_candidate_against_baseline(candidate: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "heldout_score_delta": delta(
            candidate_heldout.get("score"),
            baseline_heldout.get("score"),
        ),
        "heldout_scaffold_specificity_delta": delta(
            safe_get_metric(candidate_heldout, "scaffold_specificity_rate"),
            safe_get_metric(baseline_heldout, "scaffold_specificity_rate"),
        ),
        "val_score_delta": delta(
            candidate["val_metrics"]["score"], baseline["val_metrics"]["score"]
        ),
        "train_reads_per_second_delta": delta(
            candidate["train_metrics"]["reads_per_second"],
            baseline["train_metrics"]["reads_per_second"],
        ),
        "val_reads_per_second_delta": delta(
            candidate["val_metrics"]["reads_per_second"],
            baseline["val_metrics"]["reads_per_second"],
        ),
        ...
        "quick_test_scaffold_count_delta": delta(
            candidate_quick_test.get("scaffolds"),
            baseline_quick_test.get("scaffolds"),
        ),
...
def decide_promotion(
    ...
    if comparison["val_score_delta"] is not None and comparison["val_score_delta"] < gate.min_val_score_delta:
        hard_fail_reasons.append("validation score did not beat the baseline")

    heldout_score_delta = comparison["heldout_score_delta"]
    heldout_scaffold_specificity_delta = comparison["heldout_scaffold_specificity_delta"]
    heldout_score_pass = (
        heldout_score_delta is not None
        and heldout_score_delta > gate.min_heldout_score_delta
    )
    heldout_specificity_pass = (
        heldout_scaffold_specificity_delta is not None
        and heldout_scaffold_specificity_delta > gate.min_heldout_scaffold_specificity_delta
    )
    if heldout_score_delta is None and heldout_scaffold_specificity_delta is None:
        hard_fail_reasons.append("heldout panel evaluation missing for score/specificity gate")
    elif not heldout_score_pass and not heldout_specificity_pass:
        hard_fail_reasons.append("heldout panel did not beat baseline on score or scaffold specificity")

    if (
        heldout_scaffold_specificity_delta is not None
        and heldout_scaffold_specificity_delta < -gate.max_heldout_scaffold_specificity_drop
    ):
        hard_fail_reasons.append(
            "heldout scaffold specificity regressed beyond the allowed threshold"
        )
    ...
    if (
        ...
        and (
            matched_base_fraction_delta is None
            or matched_base_fraction_delta < 0.0
        )
    ):
        review_reasons.append("polish corrections dropped materially versus baseline")

    scaffold_gain = comparison["quick_test_scaffold_n50_delta"]
    if scaffold_gain is not None and scaffold_gain > 0:
        review_reasons.append("candidate changes scaffold continuity and needs quality review")
    ...
    return {
        ...
        "candidate_summary": {
            ...
            "heldout_score": safe_get_metric(candidate.get("heldout_metrics"), "score"),
            "heldout_scaffold_specificity_rate": safe_get_metric(
                candidate.get("heldout_metrics"), "scaffold_specificity_rate"
            ),
            "val_score": candidate["val_metrics"]["score"],
            "quick_test_elapsed_seconds": (candidate.get("quick_test") or {}).get("elapsed_seconds"),
            "quick_test_scaffold_n50_bp": (candidate.get("quick_test") or {}).get("scaffold_n50_bp"),
        },
        "baseline_summary": {
            "config": baseline["config"],
            "heldout_score": safe_get_metric(baseline.get("heldout_metrics"), "score"),
            "heldout_scaffold_specificity_rate": safe_get_metric(
                baseline.get("heldout_metrics"),
            ...
            "quick_test_scaffold_n50_bp": (baseline.get("quick_test") or {}).get("scaffold_n50_bp"),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Promote a read-mapping candidate through local eval and quick_test"
    )
    ...
    parser.add_argument("--min-primary-matches", type=int, default=3)
    parser.add_argument("--min-scaffold-matches", type=int, default=2)
    ...
    parser.add_argument("--min-heldout-scaffold-specificity-delta", type=float, default=0.0)
    parser.add_argument("--max-heldout-scaffold-specificity-drop", type=float, default=0.0)
    ...
    if not train_root.exists() or not val_root.exists() or not heldout_root.exists():
        raise FileNotFoundError(
            f"expected train/val/heldout panels under {args.tasks_root}; run read_mapping_prepare.py first"
        )

    config = load_config(args)
    gate = PromotionGate(
        min_heldout_score_delta=args.min_heldout_score_delta,
        min_heldout_scaffold_specificity_delta=args.min_heldout_scaffold_specificity_delta,
    ...
    output_dir = args.output_root / f"read_mapping_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    candidate_result = {
        ...
        "train_metrics": evaluate_read_mapping_panel(
            args.binary, train_root, config, args.position_tolerance
        ),
        "val_metrics": evaluate_read_mapping_panel(
            args.binary, val_root, config, args.position_tolerance
        ),
        "heldout_metrics": evaluate_read_mapping_panel(
            args.binary, heldout_root, config, args.position_tolerance
        ),
    }
    ...
    if args.baseline_summary is not None and args.baseline_summary.exists():
        ...
        baseline_config = MapperConfig(**DEFAULT_BASELINE_CONFIG)
        baseline_result = {
            "config": asdict(baseline_config),
            "train_metrics": evaluate_read_mapping_panel(
                args.binary, train_root, baseline_config, args.position_tolerance
            ),
            "val_metrics": evaluate_read_mapping_panel(
                args.binary, val_root, baseline_config, args.position_tolerance
            ),
            "heldout_metrics": evaluate_read_mapping_panel(
                args.binary, heldout_root, baseline_config, args.position_tolerance
            ),
            "quick_test": None,
        }

    if baseline_result.get("heldout_metrics") is None:
        baseline_config = MapperConfig(**baseline_result["config"])
        baseline_result["heldout_metrics"] = evaluate_read_mapping_panel(
            args.binary, heldout_root, baseline_config, args.position_tolerance
        )
    else:
        baseline_config = MapperConfig(**baseline_result["config"])
    ...
    heldout_gate_failed = any(
        reason.startswith("heldout panel")
        for reason in pre_decision["hard_fail_reasons"]
    )
    ...
    result = {
        ...
        "quick_test": candidate_result["quick_test"],
