#!/usr/bin/env python3
"""Promote a scaffold+polish candidate through heldout eval and quick_test."""
...
DEFAULT_OUTPUT_ROOT = (
    REPO_ROOT / "artifacts" / "autoresearch_raptor" / "scaffold_polish" / "promotions"
)
DEFAULT_TRAIN_RESULT = (
    REPO_ROOT / "artifacts" / "autoresearch_raptor" / "scaffold_polish" / "latest_result.json"
)
QUICK_TEST_READS_1 = (
...
DEFAULT_BASELINE_CONFIG = {"min_scaffold_links": 3}


@dataclass(frozen=True)
class ScaffoldPolishConfig:
    min_scaffold_links: int = 3
...
class PromotionGate:
    ...
    max_heldout_precision_drop: float = 0.0
    ...
    min_runtime_improvement_ratio_for_promotion: float = 0.01
    min_runtime_improvement_seconds_for_promotion: float = 1.0
    min_quality_improvement_epsilon: float = 1e-9


def load_config(args: argparse.Namespace) -> ScaffoldPolishConfig:
    if args.train_result is not None and args.train_result.exists():
        ...
        if winner:
            return ScaffoldPolishConfig(min_scaffold_links=winner["min_scaffold_links"])
    return ScaffoldPolishConfig(min_scaffold_links=args.min_scaffold_links)
...
def evaluate_panel(binary: Path, task_root: Path, config: ScaffoldPolishConfig) -> dict[str, Any]:
    cmd = [
        str(binary),
        "component-bench",
        "scaffold-polish",
...
def run_quick_test(
    ...
    reference_eval_input = polished_path if polished_path.exists() else scaffold_path
    ...
    result["reference_eval_target"] = "polished" if polished_path.exists() else "scaffold"
    return result
...
def compare_candidate_against_baseline(candidate: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "train_score_delta": delta(candidate["train_metrics"]["score"], baseline["train_metrics"]["score"]),
        "val_score_delta": delta(candidate["val_metrics"]["score"], baseline["val_metrics"]["score"]),
        "heldout_score_delta": delta(
            (candidate.get("heldout_metrics") or {}).get("score"),
            (baseline.get("heldout_metrics") or {}).get("score"),
        ),
        "heldout_scaffold_precision_delta": delta(
            (candidate.get("heldout_metrics") or {}).get("scaffold_link_precision"),
            (baseline.get("heldout_metrics") or {}).get("scaffold_link_precision"),
        ),
...
def decide_promotion(
    ...
    if comparison["heldout_score_delta"] is None:
        hard_fail_reasons.append("heldout panel evaluation missing for scaffold+polish gate")
    elif comparison["heldout_score_delta"] < gate.min_heldout_score_delta:
        hard_fail_reasons.append("heldout score regressed versus baseline")

    heldout_precision_delta = comparison["heldout_scaffold_precision_delta"]
    if (
        heldout_precision_delta is not None
        and heldout_precision_delta < -gate.max_heldout_precision_drop
    ):
        hard_fail_reasons.append("heldout scaffold precision regressed beyond the allowed threshold")
    ...
    if (
        ...
        hard_fail_reasons.append("aligned assembly fraction regressed beyond the allowed threshold")
    ...
    if runtime_ratio is not None and runtime_delta is not None:
        if runtime_ratio > gate.max_runtime_regression_ratio and runtime_delta > gate.max_runtime_regression_seconds:
            review_reasons.append("quick_test runtime regressed beyond the allowed threshold")

    quality_win = any(
        comparison[key] is not None and comparison[key] > gate.min_quality_improvement_epsilon
        for key in (
            "val_score_delta",
            "heldout_score_delta",
            "heldout_scaffold_precision_delta",
            "reference_coverage_fraction_delta",
            "aligned_query_fraction_delta",
            "matched_base_fraction_delta",
            "quick_test_scaffold_n50_delta",
        )
    )
    runtime_win = (
        runtime_ratio is not None
        and runtime_delta is not None
        and runtime_ratio < -gate.min_runtime_improvement_ratio_for_promotion
        and runtime_delta < -gate.min_runtime_improvement_seconds_for_promotion
    )
    if not hard_fail_reasons and not quality_win and not runtime_win:
        review_reasons.append(
            "candidate clears the non-regression gate but does not deliver a meaningful quality or runtime improvement"
        )
    ...
    return {
        ...
        "candidate_summary": {
            ...
            "heldout_scaffold_link_precision": (candidate.get("heldout_metrics") or {}).get("scaffold_link_precision"),
        ...
        "baseline_summary": {
            ...
            "heldout_scaffold_link_precision": (baseline.get("heldout_metrics") or {}).get("scaffold_link_precision"),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Promote a scaffold+polish candidate through heldout eval and quick_test"
    )
    parser.add_argument("--binary", type=Path, default=DEFAULT_BINARY)
    parser.add_argument("--tasks-root", type=Path, default=DEFAULT_TASKS_ROOT)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--train-result", type=Path, default=DEFAULT_TRAIN_RESULT)
    parser.add_argument("--min-scaffold-links", type=int, default=3)
    ...
    parser.add_argument("--max-heldout-precision-drop", type=float, default=0.0)
    ...
    if not train_root.exists() or not val_root.exists() or not heldout_root.exists():
        raise FileNotFoundError(
            f"expected train/val/heldout panels under {args.tasks_root}; run scaffold_polish_prepare.py first"
        )

    config = load_config(args)
    gate = PromotionGate(
        min_heldout_score_delta=args.min_heldout_score_delta,
        max_heldout_precision_drop=args.max_heldout_precision_drop,
    ...
    output_dir = args.output_root / f"scaffold_polish_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if args.baseline_summary is not None and args.baseline_summary.exists():
        ...
        baseline_config = ScaffoldPolishConfig(**DEFAULT_BASELINE_CONFIG)
        baseline_result = {
    ...
    if baseline_result.get("heldout_metrics") is None:
        baseline_config = ScaffoldPolishConfig(**baseline_result["config"])
        baseline_result["heldout_metrics"] = evaluate_panel(args.binary, heldout_root, baseline_config)
    else:
        baseline_config = ScaffoldPolishConfig(**baseline_result["config"])
    ...
    if args.skip_quick_test:
        ...
        if baseline_result.get("quick_test") is None:
            baseline_result["quick_test"] = {"skipped": "pending heldout gate"}
        candidate_result["quick_test"] = {"skipped": "pending heldout gate"}
    else:
    ...
    heldout_gate_failed = any(reason.startswith("heldout") for reason in pre_decision["hard_fail_reasons"])
    ...
    if args.skip_quick_test:
        ...
        if not args.skip_baseline_quick_test and baseline_result.get("quick_test", {}).get("skipped") == "pending heldout gate":
            ...
            baseline_result["quick_test"] = None
    elif heldout_gate_failed:
        candidate_result["quick_test"] = {"skipped": "heldout gate failed"}
        baseline_result["quick_test"] = {"skipped": "heldout gate failed"}
