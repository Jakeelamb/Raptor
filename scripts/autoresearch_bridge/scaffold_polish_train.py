#!/usr/bin/env python3
"""Heldout-aware exhaustive search for scaffold+polish component tasks."""
...
MIN_SCAFFOLD_LINK_VALUES = (1, 2, 3, 4, 5, 6)
...
def evaluate_config(binary: Path, task_root: Path, config: ScaffoldPolishConfig) -> dict:
    cmd = [
        str(binary),
        "component-bench",
        "scaffold-polish",
...
def enumerate_configs() -> list[ScaffoldPolishConfig]:
    configs = [ScaffoldPolishConfig()]
    configs.extend(
        ScaffoldPolishConfig(min_scaffold_links=value)
        for value in MIN_SCAFFOLD_LINK_VALUES
        if value != ScaffoldPolishConfig().min_scaffold_links
    )
    return configs
...
def selection_key(row: dict) -> tuple[float, float, float]:
    ...
    heldout_precision = (
        heldout_metrics.get("scaffold_link_precision") if heldout_metrics is not None else None
    )
    return (
        robust_score,
        float(heldout_precision) if heldout_precision is not None else 0.0,
        val_metrics["pairs_per_second"],
    )
...
def heldout_non_regressing(candidate: dict, baseline: dict) -> bool:
    ...
    if candidate_heldout["score"] < baseline_heldout["score"]:
        return False
    candidate_precision = candidate_heldout.get("scaffold_link_precision")
    baseline_precision = baseline_heldout.get("scaffold_link_precision")
    if (
...
def choose_finalists(evaluated: list[dict]) -> tuple[dict, dict, dict, int]:
    baseline = next(row for row in evaluated if row["config"] == ScaffoldPolishConfig())
    best_train = max(
        evaluated,
        key=lambda row: (row["train_metrics"]["score"], row["train_metrics"]["pairs_per_second"]),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the scaffold+polish autoresearch bridge")
    ...
    if not train_root.exists() or not val_root.exists():
        raise FileNotFoundError(
            f"expected train/val panels under {args.tasks_root}; run scaffold_polish_prepare.py first"
        )
    ...
    result = {
        ...
        "selection_policy": {
            ...
            "protected_metric": "scaffold_link_precision",
        },
    ...
    print(f"Wrote scaffold+polish result to {args.output}")
