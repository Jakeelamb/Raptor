#!/usr/bin/env python3
"""Heldout-aware deterministic grid search for error-correction component tasks."""
...
DEFAULT_OUTPUT = (
    REPO_ROOT / "artifacts" / "autoresearch_raptor" / "error_correction" / "latest_result.json"
)

MIN_COUNT_VALUES = (1, 2, 3, 4, 5)
MIN_TRUSTED_COUNT_VALUES = (3, 4, 5, 6)
SMOKE_MIN_COUNT_VALUES = (1, 2, 3)
SMOKE_MIN_TRUSTED_COUNT_VALUES = (3, 4, 5)
...
def evaluate_config(binary: Path, task_root: Path, config: ErrorCorrectionConfig) -> dict:
    cmd = [
        str(binary),
        "component-bench",
        "error-correction",
        ...
        "--min-trusted-count",
        str(config.min_trusted_count),
        "--json",
    ]
    return run_json_command(cmd)["summary"]


def enumerate_configs(smoke: bool) -> list[ErrorCorrectionConfig]:
    min_count_values = SMOKE_MIN_COUNT_VALUES if smoke else MIN_COUNT_VALUES
    min_trusted_count_values = (
        SMOKE_MIN_TRUSTED_COUNT_VALUES if smoke else MIN_TRUSTED_COUNT_VALUES
    )
    configs = [ErrorCorrectionConfig()]
    for min_count in min_count_values:
        for min_trusted_count in min_trusted_count_values:
            config = ErrorCorrectionConfig(
                min_count=min_count,
                min_trusted_count=min_trusted_count,
            )
            if config != ErrorCorrectionConfig():
...
def selection_key(row: dict) -> tuple[float, float, float, float]:
    ...
    heldout_trusted_exact = safe_metric(heldout_metrics, "trusted_target_exact_rate") or 0.0
    heldout_correction_f1 = safe_metric(heldout_metrics, "correction_f1") or 0.0
    return (
        robust_score,
        heldout_trusted_exact,
        heldout_correction_f1,
        val_metrics["kmers_per_second"],
    )
...
def evaluate_grid(
    ...
    configs: list[ErrorCorrectionConfig],
    ...
    for index, config in enumerate(configs, start=1):
        ...
        heldout_trusted_exact = safe_metric(
            leader.get("heldout_metrics"), "trusted_target_exact_rate"
        )
        print(
            f"candidates={index:03d}/{len(configs):03d} | "
            f"best_val_score={leader['val_metrics']['score']:.4f} | "
            f"best_heldout_trusted={heldout_trusted_exact if heldout_trusted_exact is not None else float('nan'):.4f} | "
...
def choose_finalists(evaluated: list[dict]) -> tuple[dict, dict, dict, int]:
    baseline = next(row for row in evaluated if row["config"] == ErrorCorrectionConfig())
    best_train = max(
        evaluated,
        key=lambda row: (row["train_metrics"]["score"], row["train_metrics"]["kmers_per_second"]),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the error-correction autoresearch bridge")
    ...
    if not train_root.exists() or not val_root.exists():
        raise FileNotFoundError(
            f"expected train/val panels under {args.tasks_root}; run error_correction_prepare.py first"
        )
    ...
    configs = enumerate_configs(args.smoke)
    ...
    print(f"Wrote error-correction result to {args.output}")
