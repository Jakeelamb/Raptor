...
| Chrysalis-equivalent clustering and graph partitioning | `src/graph/partition.rs`, `src/dist/partition.rs`, `src/graph/builder.rs`, `src/pipeline/large_genome_assembler.rs` weighted graph functions, `raptor_components.json` and `raptor_component_graphs.json` from `raptor trinity` | First component graph artifact exists: assembled transcripts are clustered by shared sequence, workflow reads/pairs are assigned back to components, and graph edges now report supporting reads, pairs, and observed shared k-mers. The tiny fixture clusters both related transcripts into one component graph with two contig nodes, one supported shared-sequence edge, and full component read/pair support. This is still not Trinity Chrysalis-style complete de Bruijn graph construction from reads. | 1 | Replace shared-sequence transcript graphing with read/k-mer-built component graph construction and prove component membership/read assignment against Trinity on small fixtures |
...
Interpretation: the two related assembled transcripts are grouped into one deterministic shared-sequence component graph on the tiny fixture, every workflow read pair is assigned back to that component, and the component edge has direct read/pair/k-mer support. This is a useful first Chrysalis-shaped graph artifact, but it is not yet Trinity Chrysalis parity because the graph is still seeded by assembled transcript overlap rather than built as a complete read/k-mer de Bruijn component, and component membership has not been compared against Trinity.
...
2. Replace shared-sequence transcript graphing with read/k-mer-built component graph construction and compare membership/read assignment against Trinity.
