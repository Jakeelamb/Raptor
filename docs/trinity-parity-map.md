...
| --- | --- | --- | --- | --- |
| Input FASTQ handling | `src/io/fastq.rs`, checked streaming readers, gzip-capable callers, `raptor trinity --input1 [--input2]`, `bench/trinity_parity/run_tiny_fixture.py` | Real infrastructure exists for single-end and paired-end FASTQ. The candidate panel now gates single-end `raptor trinity` on tiny_alt_isoform and ambiguous_paralog, and paired-end workflow on all current fixtures. It still needs malformed FASTQ, sample-file, and Trinity-style CLI compatibility cases. | 2 | Add parity fixtures covering malformed FASTQ, Trinity-style input combinations, sample files, and strand-specific orientation |
