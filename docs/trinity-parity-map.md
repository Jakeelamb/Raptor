...
- candidate panel gate: `python3 bench/trinity_parity/run_panel.py` runs 6 fixture/insert combinations and writes `target/trinity_parity/candidate_panel/panel_report.json`. Current result passes all cases: tiny insert sweep selected lengths [252,240] with precision/recall/F1 1.0 for inserts 110,140,160,180; ambiguous paralog selected lengths [240,234,240] with precision/recall/F1 1.0; compact fusion selected lengths [216,216] with precision/recall/F1 1.0. Current workflow resource evidence on this host uses `procfs_process_group_poll`: elapsed 0.222-0.335 s and peak observed RSS 61,172-66,472 KB across the 6 Raptor workflow runs. Current workflow output footprint is 12 files and 118,624-185,902 bytes per run, with final FASTA size 181-253 bytes. `nvidia-smi` reports an NVIDIA GeForce RTX 5070 Laptop GPU visible to the harness; current tiny workflow runs show 116 MiB max GPU memory, 0% max GPU utilization, and 4.92-6.36 W max power draw, so this is telemetry evidence, not evidence of GPU acceleration.
