...
| Butterfly-equivalent isoform reconstruction | `src/pipeline/isoform_processor.rs`, `src/graph/isoform_graph.rs`, `src/graph/isoform_traverse.rs`, `src/graph/transcript.rs`, `src/io/transcript_io.rs`, `tests/isoform_expression_properties.rs`, `benches/isoform_paths.rs`, component `read_kmer_paths` in `raptor_component_graphs.json`, `raptor_component_paths.fasta` and `raptor_component_selected_isoforms.fasta` from `raptor trinity` | Directed path traversal, transcript writers, first component read-kmer path extraction, component transcript candidates, and production selected component isoform FASTA emission exist. The selected isoforms recover the tiny truth/oracle exactly, but selection is still conservative component-contig carry-through rather than a scored read/read-pair-supported graph path decision. README language calling this "Butterfly-like" should remain provisional until benchmarked. | 1 | Add graph/read-pair-supported isoform scoring, truth-known alternative-splicing fixtures, read-pair path evidence, paralog/fusion stress cases, and compare isoform precision/recall/F1 to Trinity |
