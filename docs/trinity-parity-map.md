...
| --- | --- | --- | --- | --- |
| Input FASTQ handling | `src/io/fastq.rs`, checked streaming readers, gzip-capable callers, `raptor trinity --input1 [--input2]`, comma-separated direct input lists, `raptor trinity --samples-file`, `raptor trinity --SS_lib_type`, `bench/trinity_parity/run_tiny_fixture.py` | Real infrastructure exists for single-end and paired-end FASTQ. The candidate panel now gates single-end `raptor trinity` on tiny_alt_isoform and ambiguous_paralog, paired-end workflow on all current fixtures, one-row and multi-row Trinity-style condition/replicate/left/right sample files on all current fixtures, comma-separated direct read lists on all current fixtures, `--SS_lib_type RF` validation/reporting on all current fixtures, and malformed FASTQ rejection with nonzero CLI exit plus FASTQ/quality error context on all 6 fixture/insert runs. It still needs strand-aware orientation effects in graph/isoform reconstruction rather than only a validated workflow contract. | 2 | Add parity fixtures proving strand-specific orientation changes biological output where expected |
...
| Strand-specific RNA-seq | `raptor trinity --SS_lib_type`, `ss_lib_type` in `raptor_trinity_report.json`, stranded RF candidate-panel gate | The workflow now accepts and validates Trinity-style `F`, `R`, `FR`, and `RF` library codes and preserves the normalized code in the report. This is only a CLI/reporting contract; assembly graph orientation and isoform reconstruction are not yet strand-aware. | 0 | Add explicit strand-orientation model and fixtures where strand-specific mode changes read assignment and isoform selection |
