...
| --- | --- | --- | --- | --- |
| Input FASTQ handling | `src/io/fastq.rs`, checked streaming readers, gzip-capable callers, `raptor trinity --input1 [--input2]`, comma-separated direct input lists, `raptor trinity --samples-file`, `raptor trinity --SS_lib_type`, `bench/trinity_parity/run_tiny_fixture.py` | Real infrastructure exists for single-end and paired-end FASTQ. The candidate panel now gates single-end `raptor trinity` on tiny_alt_isoform and ambiguous_paralog, paired-end workflow on all current fixtures, one-row and multi-row Trinity-style condition/replicate/left/right sample files on all current fixtures, comma-separated direct read lists on all current fixtures, `--SS_lib_type RF` read-orientation materialization/reporting on all current fixtures, and malformed FASTQ rejection with nonzero CLI exit plus FASTQ/quality error context on all 6 fixture/insert runs. It still needs richer strand-aware graph/isoform stress cases beyond RF orientation recovery. | 2 | Add parity fixtures where strand-specific orientation disambiguates overlapping antisense transcripts |
...
| Strand-specific RNA-seq | `raptor trinity --SS_lib_type`, `ss_lib_type` and `assembly_input*` in `raptor_trinity_report.json`, strand-oriented workflow FASTQs, stranded RF candidate-panel gate | The workflow accepts and validates Trinity-style `F`, `R`, `FR`, and `RF` library codes, preserves the normalized code in the report, and materializes RF paired reads or R single-end reads into workflow-owned reverse-complemented FASTQs before assembly. The candidate panel now uses RF-oriented synthetic reads and gates forward-strand selected precision/F1, so reverse-complement-only output is no longer sufficient. This is still not complete strand-specific parity because antisense-overlap disambiguation has not been benchmarked against Trinity. | 1 | Add explicit antisense/overlap fixtures where strand-specific mode changes read assignment and isoform selection, then compare to Trinity |
