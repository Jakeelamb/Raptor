...
Interpretation: the two related assembled transcripts are grouped into one deterministic component graph on the tiny fixture using `sequence_or_read_kmer` clustering, every workflow read pair is assigned back to that component, the component edge has direct read/pair/k-mer support, and the component serializes a full assigned-read k-mer graph for this component plus a capped edge sample for quick reporting. This is a useful Chrysalis-shaped graph artifact, but it is not yet Trinity Chrysalis parity because Trinity component membership comparison and graph-driven isoform reconstruction are still missing.
...
2. Compare component membership/read assignment against Trinity, then feed the full read-kmer component graph into path reconstruction instead of only emitting it as an artifact.
