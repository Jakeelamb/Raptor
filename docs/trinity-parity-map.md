...
| Chrysalis-equivalent clustering and graph partitioning | `src/graph/partition.rs`, `src/dist/partition.rs`, `src/graph/builder.rs`, `src/pipeline/large_genome_assembler.rs` weighted graph functions, `raptor_components.json` and `raptor_component_graphs.json` from `raptor trinity` | First component graph artifact exists: assembled transcripts are clustered by shared sequence, workflow reads/pairs are assigned back to components, graph edges report supporting reads/pairs/observed shared k-mers, and each component graph now reports read-derived k-mer node/edge counts from assigned reads. This is still not Trinity Chrysalis-style complete de Bruijn graph construction from reads because the component boundary is still seeded by transcript overlap. | 1 | Replace shared-sequence transcript component boundaries with read/k-mer-built component graph construction and prove component membership/read assignment against Trinity on small fixtures |
...
2. Replace shared-sequence transcript component boundaries with read/k-mer-built component graph construction and compare membership/read assignment against Trinity.
