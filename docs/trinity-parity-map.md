...
| Butterfly-equivalent isoform reconstruction | `src/pipeline/isoform_processor.rs`, `src/graph/isoform_graph.rs`, `src/graph/isoform_traverse.rs`, `src/graph/transcript.rs`, `src/io/transcript_io.rs`, `tests/isoform_expression_properties.rs`, `benches/isoform_paths.rs`, component `read_kmer_paths` in `raptor_component_graphs.json` | Directed path traversal, transcript writers, and first component read-kmer path extraction exist. Current path support is still not used to emit full-length isoforms from component graphs and does not yet prove read/read-pair-supported isoform recovery or paralog separation. README language calling this "Butterfly-like" should remain provisional until benchmarked. | 1 | Feed component read-kmer paths into transcript reconstruction, add truth-known alternative-splicing fixtures, read-pair path evidence, paralog/fusion stress cases, and compare isoform precision/recall/F1 to Trinity |
...
2. Compare component membership/read assignment against Trinity, then feed component read-kmer paths into transcript reconstruction instead of only emitting them as an artifact.
