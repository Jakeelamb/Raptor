...
| In silico normalization | `src/pipeline/normalize.rs`, `src/kmer/normalize.rs`, `src/bin/normalize_reads.rs`, `src/bin/normalize_paired_reads.rs`, `normalize` CLI, `bench/trinity_parity/run_tiny_fixture.py` | Two-pass CMS/ntHash normalization exists for single and paired reads. `NormalizeConfig` now centralizes k, target coverage, min abundance, max read/pair limit, and GPU request state for the CLI and standalone binaries. The candidate panel now runs Raptor paired normalization by default and, when Trinity is enabled, records Trinity `insilico_read_normalization` kept-pair counts/fractions plus retained original-pair overlap. Small fixtures require exact kept-pair retention. The high-depth normalization fixture forces read reduction at target 200 and currently measures Raptor 1703/5100 kept pairs versus Trinity 1697/5100 kept pairs, with a tracked candidate delta gate of 0.02. Retained-pair identity is still weak: 570 overlapping pairs, Jaccard 0.201413. Normalized-output biological gates now include one high-depth 900 bp transcript and one high-depth two-isoform fixture; both currently match Trinity at assembly F1 1.0, with the isoform fixture recovering [336,324]. The Trinity-like workflow now defaults normalization `--min-abundance` to 1 rather than inheriting the standalone error-filter default of 2; on the public source dataset this keeps 30,258/30,575 pairs instead of 23,898/30,575 and moves final Raptor-vs-Trinity F1 from 0.019231 to 0.052174. This is still not full normalization parity because retained-read identity is not matched, biological-effect gates are synthetic, and GPU normalization is still a logged CPU fallback. | 1 | Improve retained-read identity and expand normalized-output biological-effect gates to real data, then add CPU/GPU output-equivalence if GPU normalization becomes real |
...
| Inchworm-equivalent k-mer contig construction | `src/graph/assembler.rs`, `src/kmer/*`, `src/pipeline/assemble.rs`, `src/pipeline/large_genome_assembler.rs`, `tests/greedy.rs`, `benches/greedy_assembly.rs` | Greedy/adaptive k-mer assembly, a larger de Bruijn-style genome path, and an oriented k-mer path fallback now exist. The oriented fallback only triggers when canonical graph output is fragmented below `--min-len`, because canonical k-mers cannot safely be walked as oriented transcript sequence. It now tries the adaptive k and Trinity-style k=25, builds the fallback graph from read-observed adjacent k-mer transitions, extends contig boundaries inside the oriented graph, then keeps the candidate set with the best N50/longest/total-bases key. On `trinity_source_test_assembly`, this produces Inchworm-scale contiguity: 41 contigs, N50 5,381, max length 8,723, and 26 contigs >=1kb. Against Trinity final FASTA, strict reciprocal F1 is now 0.052174; observed-edge construction adds one exact containment hit below the hard gate but does not move the strict F1 gate. This is still not Inchworm parity because Trinity emits 1,275 Inchworm sequences in the fresh run and final Raptor-vs-Trinity transcript F1 remains far below 0.9. | 1 | Tighten oriented path selection and compare against Trinity/public outputs until reciprocal FASTA matches clear the frozen gate |
| Chrysalis-equivalent clustering and graph partitioning | `src/graph/partition.rs`, `src/dist/partition.rs`, `src/graph/builder.rs`, `src/pipeline/large_genome_assembler.rs` weighted graph functions, `raptor_components.json` and `raptor_component_graphs.json` from `raptor trinity` | First component graph artifact exists. Workflow components now use `sequence_or_read_kmer` clustering for exhaustive-sized outputs, so observed read k-mers can define component boundaries when sequence overlap alone is below threshold. Reads/pairs are assigned back to components, graph edges report supporting reads/pairs/observed shared k-mers, and each component graph serializes the full assigned-read k-mer node/edge records plus non-branching read-kmer paths. Public-scale outputs above the exhaustive graph cap now emit a cheap sequence-only component summary instead of empty component JSON; on `trinity_source_test_assembly`, the 41-contig output reports 23 sequence-overlap components while skipping expensive full read-kmer graphs. This is still not Trinity Chrysalis parity because component membership/read assignment has not been compared against Trinity stage artifacts or a frozen oracle. | 1 | Prove component membership/read assignment against Trinity, then use bounded read-kmer paths for isoform reconstruction rather than only reporting or skipping them |
...
| Evaluation and reporting | `src/eval/metrics.rs`, `src/eval/gtf_compare.rs`, `gtf-compare`, `eval`, `stats`, `bench/README.md`, `bench/trinity_parity/run_tiny_fixture.py`, `bench/trinity_parity/run_panel.py`, `bench/trinity_parity/panel.json`, `bench/trinity_parity/public_panel.json`, `bench/trinity_parity/validate_public_panel.py`, `bench/trinity_parity/plan_public_panel.py`, `bench/trinity_parity/materialize_public_panel.py`, `bench/trinity_parity/run_public_panel.py`, `scripts/trinity_docker.sh`, `src/pipeline/trinity_workflow.rs` | Length metrics, GTF precision/recall, a tiny paired-end Raptor/Trinity harness, a deterministic candidate-panel runner, selected isoform reciprocal-match precision/recall/F1 gates, per-command elapsed/RSS reporting, output byte/file footprint reporting, `nvidia-smi` GPU telemetry, CPU-vs-GPU-requested selected isoform equivalence gates, single-end selected-output gates, OpenCL k-mer workflow telemetry, a JSON report from `raptor trinity`, and workflow component-count reporting exist. Current GPU-requested panel evidence is selected-output precision/F1 1.0, elapsed 0.540-0.671 s, RSS 332,164-334,052 KB, GPU memory 133-446 MiB, and 0-18% sampled utilization on tiny fixtures. The harness now captures reciprocal Raptor-vs-Trinity selected-isoform precision/recall/F1 when both FASTAs exist and can enforce per-fixture `min_trinity_selected_f1` gates when Trinity is enabled; it now fails gated datasets when the required Raptor-vs-Trinity FASTA comparison is missing instead of accepting absent Trinity output as a pass. With `trinityrnaseq/trinityrnaseq:2.15.2`, tiny_alt_isoform and both antisense fixtures are enforced at selected-output F1 0.95 and currently match at F1 1.0; ambiguous_paralog and compact_fusion remain recorded divergence probes until their policy is decided. A proposed public-panel manifest now declares three real/public cases and schema-gates required metrics, resource thresholds, inputs, and Trinity-stage coverage; the run-plan tool resolves local inputs and emits exact Raptor/Trinity commands, the materializer stages the two GitHub-backed datasets at pinned commits, and the public runner can execute ready cases with timeout/resource/output reporting. The public runner now reports threshold sweeps, top exact-containment matches, and aggregate containment summaries for final Trinity FASTA and Trinity Inchworm FASTA separately. With oriented k-mer paths, read-observed oriented graph edges, graph-boundary extension, Trinity-style k=25 candidate selection, bounded public component artifacts, and workflow `--min-abundance 1`, Raptor emits 41 contigs (N50 5,381, max 8,723), Trinity final emits 74 transcripts (N50 5,399, max 8,973), final reciprocal FASTA F1 is now 0.052174 at 0.95 coverage, and observed exact Raptor-to-Trinity containment is 10 hits, but this remains far below the required 0.9. | 1 | Tighten oriented path selection/isoform reconstruction until public reciprocal matches clear the frozen gates |
...
5. Materialize and run the proposed public benchmark panel, then promote it from `proposed_frozen_v0` only after approval and recorded Raptor/Trinity outputs.
