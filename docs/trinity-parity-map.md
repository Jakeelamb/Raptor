...
| Inchworm-equivalent k-mer contig construction | `src/graph/assembler.rs`, `src/kmer/*`, `src/pipeline/assemble.rs`, `src/pipeline/large_genome_assembler.rs`, `tests/greedy.rs`, `benches/greedy_assembly.rs` | Greedy/adaptive k-mer assembly, a larger de Bruijn-style genome path, and an oriented k-mer path fallback now exist. The oriented fallback only triggers when canonical graph output is fragmented below `--min-len`, because canonical k-mers cannot safely be walked as oriented transcript sequence. On `trinity_source_test_assembly`, this produces Inchworm-scale contiguity: 31 contigs, N50 3,532, max length 8,756, and 12 contigs >=1kb. Against Trinity `inchworm.DS.fa`, Raptor now has 2 reciprocal strict 0.95-coverage matches and 4 reciprocal 0.90-coverage matches. This is still not Inchworm parity because Trinity emits 1,282 Inchworm sequences and final Raptor-vs-Trinity transcript F1 remains 0.0. | 1 | Tighten oriented path selection and compare against Trinity/public outputs until reciprocal FASTA matches clear the frozen gate |
