...
- ambiguous paralog fixture at insert 160: truth transcripts [240,234,240], workflow selected lengths [240,240,234,240], truth min coverage 1.0, selected precision 0.75, recall 1.0, F1 0.857143, false positives 1, false negatives 0, 1 component, 5 component graph edges, 20 isoform candidates, 4 selected contig candidates, 16 rejected read-kmer path candidates
...
The first ambiguous isoform/paralog fixture now exists and passes the workflow gate at insert 160, recovering all three truth transcripts while surfacing one extra selected 240 bp contig. The harness now measures that directly: selected isoform precision 0.75, recall 1.0, F1 0.857143, false positives 1, false negatives 0 at reciprocal match coverage 0.95. That is useful pressure on candidate scoring, but the precision threshold is only a rescue baseline; final parity needs a much stricter threshold and Trinity comparison.
...
2. Tighten selected isoform precision/F1 by removing the ambiguous false-positive contig, then expand from the ambiguous paralog fixture to fusion stress and Trinity component/read-assignment comparison.
