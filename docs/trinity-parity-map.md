...
| Butterfly-equivalent isoform reconstruction | `src/pipeline/isoform_processor.rs`, `src/graph/isoform_graph.rs`, `src/graph/isoform_traverse.rs`, `src/graph/transcript.rs`, `src/io/transcript_io.rs`, `tests/isoform_expression_properties.rs`, `benches/isoform_paths.rs`, component `read_kmer_paths` in `raptor_component_graphs.json`, `raptor_component_paths.fasta`, `raptor_component_selected_isoforms.fasta`, and `raptor_component_selected_isoforms.json` from `raptor trinity` | Directed path traversal, transcript writers, first component read-kmer path extraction, component transcript candidates, production selected component isoform FASTA emission, and selected-isoform evidence JSON exist. The selected isoforms recover the tiny truth/oracle exactly and now carry deterministic `component_contig_evidence_score_v1` ranks from direct read, pair, and read-kmer-path support, but selection is still conservative component-contig carry-through rather than true graph path pruning. README language calling this "Butterfly-like" should remain provisional until benchmarked. | 1 | Use the score to choose among competing graph/path candidates, add truth-known alternative-splicing fixtures, read-pair path evidence, paralog/fusion stress cases, and compare isoform precision/recall/F1 to Trinity |
...
2. Use selected-isoform evidence scores to choose among competing graph/path candidates, then compare component membership/read assignment against Trinity.
