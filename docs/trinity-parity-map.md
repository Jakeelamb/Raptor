...
- compact fusion fixture at insert 160: truth transcripts [216,216], workflow selected lengths [336], truth min coverage 1.0 but selected precision 0.0, recall 0.0, F1 0.0, false positives 1, false negatives 2, 1 component, 0 component graph edges, 3 isoform candidates. This is a failing benchmark by design until paired-path evidence prevents the fusion.
...
The compact-overlap fusion fixture now exists and fails for the right reason: coverage-only recovery is 1.0 because the fused 336 bp contig contains both 216 bp truth transcripts, but reciprocal selected-isoform precision/recall/F1 are all 0.0. This is the next concrete paired-path blocker.
...
2. Use paired-path evidence to split or reject the compact fusion fixture, then compare Trinity component/read assignment.
