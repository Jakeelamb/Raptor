...
| Evaluation and reporting | `src/eval/metrics.rs`, `src/eval/gtf_compare.rs`, `gtf-compare`, `eval`, `stats`, `bench/README.md`, `bench/trinity_parity/run_tiny_fixture.py`, `bench/trinity_parity/run_panel.py`, `bench/trinity_parity/panel.json`, `src/pipeline/trinity_workflow.rs` | Length metrics, GTF precision/recall, a tiny paired-end Raptor/Trinity harness, a deterministic candidate-panel runner, selected isoform reciprocal-match precision/recall/F1 gates, per-command elapsed/RSS reporting, a JSON report from `raptor trinity`, and workflow component-count reporting exist. The harness can freeze a live Trinity output as oracle when Trinity is installed. There is still no frozen public panel, BUSCO/read-representation/full-length recovery integration, disk capture, or GPU capture. | 1 | Expand `bench/trinity_parity/` into exact versions, commands, metrics, disk/GPU capture, and reproducible public-panel reports |
