...
| Butterfly-equivalent isoform reconstruction | `src/pipeline/isoform_processor.rs`, `src/graph/isoform_graph.rs`, `src/graph/isoform_traverse.rs`, `src/graph/transcript.rs`, `src/io/transcript_io.rs`, `tests/isoform_expression_properties.rs`, `benches/isoform_paths.rs`, component `read_kmer_paths` in `raptor_component_graphs.json`, `raptor_component_paths.fasta` from `raptor trinity` | Directed path traversal, transcript writers, first component read-kmer path extraction, and a component transcript-candidate FASTA exist. The component FASTA includes assembled component contigs plus read-kmer path candidates and recovers the tiny truth/oracle, but final reported transcripts are still not selected from read/read-pair-supported component graph paths. README language calling this "Butterfly-like" should remain provisional until benchmarked. | 1 | Use component graph/read-pair evidence to select final isoforms, add truth-known alternative-splicing fixtures, read-pair path evidence, paralog/fusion stress cases, and compare isoform precision/recall/F1 to Trinity |
...
Interpretation: the two related assembled transcripts are grouped into one deterministic component graph on the tiny fixture using `sequence_or_read_kmer` clustering, every workflow read pair is assigned back to that component, the component edge has direct read/pair/k-mer support, and the workflow now emits component transcript candidates that recover the tiny truth/oracle while also exposing deterministic non-branching read-kmer graph paths. This is a useful Chrysalis/Butterfly-shaped artifact, but it is not yet Trinity parity because Trinity component membership comparison and final isoform selection from graph/read-pair evidence are still missing.
...
2. Compare component membership/read assignment against Trinity, then use component graph/read-pair evidence to select final isoforms instead of carrying both contig and path candidates.
