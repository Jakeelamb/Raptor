...
| Chrysalis-equivalent clustering and graph partitioning | `src/graph/partition.rs`, `src/dist/partition.rs`, `src/graph/builder.rs`, `src/pipeline/large_genome_assembler.rs` weighted graph functions, `raptor_components.json` from `raptor trinity` | First `RaptorComponent` artifact exists: assembled transcripts are clustered by shared sequence, workflow reads/pairs are assigned back to components, and the Trinity-like workflow emits the JSON artifact. The tiny fixture clusters both related transcripts into one component with full read/pair support. This is still not Trinity Chrysalis-style complete component de Bruijn graph construction. | 1 | Replace shared-sequence clustering with graph-backed component construction, emit component graph artifacts, and prove component membership/read assignment against Trinity on small fixtures |
...
Interpretation: the two related assembled transcripts are grouped into one deterministic shared-sequence component on the tiny fixture, and every workflow read pair is assigned back to that component. This is a useful first Chrysalis-shaped artifact, but it is not yet Trinity Chrysalis parity because no complete per-component de Bruijn graph is emitted and component membership has not been compared against Trinity.
...
2. Replace shared-sequence component grouping with graph-backed component construction and compare membership/read assignment against Trinity.
