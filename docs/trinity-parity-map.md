...
| Chrysalis-equivalent clustering and graph partitioning | `src/graph/partition.rs`, `src/dist/partition.rs`, `src/graph/builder.rs`, `src/pipeline/large_genome_assembler.rs` weighted graph functions, `raptor_components.json` from `raptor trinity` | First `RaptorComponent` artifact exists: assembled transcripts are clustered by shared sequence and emitted by the Trinity-like workflow. The tiny fixture clusters both related transcripts into one component. This is only a deterministic transcript-component report; it is not yet Trinity Chrysalis-style read partitioning or complete component de Bruijn graph construction. | 1 | Replace shared-sequence clustering with graph/read-backed component construction, emit component graph artifacts, and prove component membership/read assignment against Trinity on small fixtures |
...
Interpretation: the two related assembled transcripts are grouped into one deterministic shared-sequence component on the tiny fixture. This is a useful first Chrysalis-shaped artifact, but it is not yet Trinity Chrysalis parity because reads are not assigned into component graphs and no complete per-component de Bruijn graph is emitted.
