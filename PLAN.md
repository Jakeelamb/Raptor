...
- `raptor trinity` now also emits `raptor_components.json` and `raptor_component_graphs.json`. On the tiny paired-end workflow fixture it reports 1 component containing contigs [0,1] with 492 total bases, 30 assigned reads, 15 assigned pairs, and a 2-node/1-edge component graph with a 91 bp edge supported by 10 reads, 10 pairs, and 133 observed shared k-mers. This is first read/k-mer-backed Chrysalis-shaped graph evidence, not Chrysalis parity: it is still shared-transcript graphing plus read assignment, not complete component de Bruijn graph partitioning.
