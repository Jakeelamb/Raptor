...
- Normalization now has a shared `NormalizeConfig` pipeline path used by `raptor normalize`, `normalize_reads`, and `normalize_paired_reads`; CLI `coverage_target`, `max_reads`, and GPU request state are no longer silently dropped. This is codepath cleanup, not Trinity normalization parity until kept-read behavior is compared to Trinity.
