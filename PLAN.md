...
- The harness now has a failing `--fixture compact_fusion` gate. At insert 160 it generates two overlapping 216 bp truth transcripts, but `raptor trinity` emits one selected 336 bp fused contig. Coverage-only truth recovery is misleadingly 1.0, while selected precision/recall/F1 are 0.0 with 1 false positive and 2 false negatives. This is the next paired-path blocker.
