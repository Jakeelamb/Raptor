...
- `raptor trinity --SS_lib_type` now accepts and reports Trinity-style `F`, `R`, `FR`, and `RF` codes. The workflow materializes R single-end and RF paired-end reads into strand-oriented FASTQs before assembly and records actual `assembly_input*` paths in the report. The candidate panel gates `--SS_lib_type RF` across all 6 fixture/insert runs using RF-oriented synthetic reads and requires forward-strand selected precision/F1 1.0; antisense-overlap disambiguation is still missing.
