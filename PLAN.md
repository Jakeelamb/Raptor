...
- `bench/trinity_parity/run_tiny_fixture.py` now generates a tiny truth-known alternative-isoform fixture, runs current `raptor assemble`, checks both generated truth and the frozen oracle at `bench/trinity_parity/oracles/tiny_alt_isoform.fa`, can run paired-end Trinity with optional oracle freeze when Trinity is installed, can exercise Raptor normalize -> assemble with kept-pair metrics, can run the single-command `raptor trinity` workflow in paired, GPU-requested paired, selected single-end, Trinity-style samples-file, and comma-separated direct read-list modes, and can run malformed FASTQ negative checks. `bench/trinity_parity/run_panel.py` now runs the current candidate panel from `bench/trinity_parity/panel.json` and writes one machine-readable panel report with elapsed time, RSS, output-footprint, GPU telemetry, CPU-vs-GPU-requested output-equivalence metrics, single-end selected-isoform metrics, samples-file/comma-list selected-isoform metrics, and malformed FASTQ rejection metrics. GPU-requested workflow runs now build with `--features gpu` and exercise OpenCL k-mer counting where available, while graph build remains CPU.
