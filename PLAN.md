...
- `raptor trinity` now also emits `raptor_components.json`. On the tiny paired-end workflow fixture it reports 1 component containing contigs [0,1] with 492 total bases, 30 assigned reads, and 15 assigned pairs. This is first read-backed Chrysalis-shaped component evidence, not Chrysalis parity: it is still shared-sequence grouping plus read assignment, not complete component de Bruijn graph partitioning.
