...
- `raptor trinity --SS_lib_type` now accepts and reports Trinity-style `F`, `R`, `FR`, and `RF` codes. The candidate panel gates `--SS_lib_type RF` across all 6 fixture/insert runs and verifies the report preserves `RF`; this is not yet full strand-aware graph/isoform behavior.
