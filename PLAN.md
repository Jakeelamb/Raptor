...
- `bench/trinity_parity/run_tiny_fixture.py` now generates a tiny truth-known alternative-isoform fixture, runs current `raptor assemble`, checks both generated truth and the frozen oracle at `bench/trinity_parity/oracles/tiny_alt_isoform.fa`, can run paired-end Trinity with optional oracle freeze when Trinity is installed, and can exercise Raptor normalize -> assemble with kept-pair metrics.
