...
- The harness now has `--fixture ambiguous_paralog`. At insert 160 it generates truth transcript lengths [240,234,240], and `raptor trinity` emits selected lengths [240,240,234,240] with truth min coverage 1.0, 1 component, 5 component graph edges, 20 scored isoform candidates, 4 selected contig candidates, and 16 rejected read-kmer path candidates. This adds real stress beyond the tiny two-isoform case, but also exposes the next precision/F1 gap: recall is perfect on the fixture, selected output has one extra 240 bp contig.
