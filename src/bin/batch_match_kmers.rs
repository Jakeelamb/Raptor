...
fn main() -> io::Result<()> {
    ...
    if args.mismatches > args.min_overlap / 2 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
                "Maximum mismatches ({}) should not exceed half the minimum overlap length ({})",
                args.mismatches,
                args.min_overlap / 2
            ),
        ));
    ...
    let queries: Vec<String> = reader.lines().map_while(Result::ok).collect();
    ...
    let targets: Vec<String> = reader.lines().map_while(Result::ok).collect();
    ...
    for (q_idx, query) in queries.iter().enumerate() {
        ...
        for (t_idx, (overlap, mismatch_count)) in matches.iter().enumerate() {
            if let Some((overlap_size, shift)) = overlap {
                writeln!(
                    writer,
                    "{},{},{},{},{}",
                    q_idx, t_idx, overlap_size, shift, mismatch_count
                )?;
                total_matches += 1;
            }
        }
        ...
        if (q_idx + 1) % 100 == 0 || q_idx + 1 == queries.len() {
            println!(
                "Processed {}/{} queries, found {} matches so far",
                q_idx + 1,
                queries.len(),
                total_matches
            );
