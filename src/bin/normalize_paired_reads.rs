...
fn main() {
    ...
    let k = if args.len() > 4 {
        args[4].parse().unwrap_or(DEFAULT_K)
    } else {
        DEFAULT_K
    };
    let target = if args.len() > 5 {
        args[5].parse().unwrap_or(DEFAULT_COVERAGE_TARGET)
    } else {
        DEFAULT_COVERAGE_TARGET
    };
    let min_abund = if args.len() > 6 {
        args[6].parse().unwrap_or(DEFAULT_MIN_ABUNDANCE)
    } else {
    ...
    if gpu_enabled {
        ...
        println!(
            "Memory efficient mode: Processing in chunks of {} read pairs",
            CHUNK_SIZE
        );
        ...
        loop {
            ...
            let sequences: Vec<String> = chunk
                .iter()
            ...
            println!(
                "Processed {} read pairs, kept {} so far",
                total_pairs, kept_pairs
            );
        ...
        println!(
            ...
            kept_pairs,
            total_pairs,
            (kept_pairs as f64 / total_pairs as f64) * 100.0
        );
...
fn print_usage(program_name: &str) {
    ...
    eprintln!(
        "  {} R1.fastq.gz R2.fastq.gz output 21 50 2 --gpu",
        program_name
    );
    eprintln!();
