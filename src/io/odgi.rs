...
pub fn export_odgi_json(
    ...
    for (id, segments) in path_map {
        ...
        metadata.insert(
            "has_inversions".to_string(),
            if has_inversions { 1.0 } else { 0.0 },
        );
...
pub fn export_path_metadata_json(metadata: &[PathMetadata], output: &str) -> io::Result<()> {
    let file = File::create(output)?;
    ...
    for (i, path) in metadata.iter().enumerate() {
        ...
        writeln!(
            writer,
            "      \"has_inversions\": {}",
            if path.has_inversions { "true" } else { "false" }
        )?;
